...
class HMM:
    def __init__(self, data, ins=None, k=5, TM=None, OM=None, full_cov=False, io=True, state_io=False,
                 personalized_io=False, personalized=False, eps=0, min_var=1e-6, device='cuda:0', priorV=False,
        ...
        self.data = data.to(device=self.device)  # n x t x d      #3维数据
        self.n, self.t, self.d = self.data.shape                   #n,t,d = 三个维度

        #store likelihood (this is the objective if no personalized effects are used)
        self.ll = []     #ll 似然

        if self.perso_io and self.perso:     # if io effects should be personalized and if personalized (non-io) effects should be included
            # case with both personalized state and medication effects
            self.elbo = [] #objective
            self.mu_hat = torch.zeros(self.n, self.d, requires_grad=True, device=self.device)    #mu_hat = torch.zeros（n*d）
            ...
            if self.VI_diag:
                ...
                self.L_hat = torch.zeros(self.n, self.d, self.d, device=self.device)    # L_hat =torch.zeros(n,d,d)
                self.L_hat[torch.stack([torch.eye(self.d) for _ in range(self.n)]) == 1] = self.tril_vec    #torch.stack堆叠
            else:
                ...
                self.L_hat[torch.tril(torch.ones(self.n, self.d, self.d)) == 1] = self.tril_vec   #torch.tril(b, diagonal=1) 包含对角线下三角，torch.tril(b, diagonal=-1)不包含对角线下三角
                 #L_hat存放tril_vec

            self.nu_hat = torch.zeros(self.n, self.d, requires_grad=True, device=self.device)   #nu_hat = torch.zeros(n,d)
            # vector for optimization of covariance which is mapped into lower triangular cholesky factor
            if self.VI_diag:
        ...
        if self.io:  # flag indicating if the model is an input-output HMM; drugs should not be none if io=True
            self.ins = ins.to(self.device)  # n x t x 1

        # store the time mask
        if TM is None:
            self.tm = torch.ones(self.n, self.t, requires_grad=False, device=self.device)  #tm = torch.ones(n,t)
        else:
    ...
    def initialize_model(self, km_init=True):
        ...
        if self.full_cov:
            ...
            var = torch.Tensor(self.k, self.d).fill_(self.ini_var)
        ...
        # transition matrix
        if self.ut:       #flag to indicate upper triangular structure for transition matrix
